...
class Service<T> {
  ...
  Future<Map<String, dynamic>> createEntity(String url, T entity) =>
      http.post(Uri.parse(url), headers: _headers, body: jsonEncode(entity)).then((response) => _handleErrors(response));

  Future<Map<String, dynamic>> updateEntity(String url, T entity) =>
      http.put(Uri.parse(url), headers: _headers, body: jsonEncode(entity)).then((response) => _handleErrors(response));

  Future<Map<String, dynamic>> getEntity(String url) => http.get(Uri.parse(url)).then((response) => _handleErrors(response));

  Future<Map<String, dynamic>> deleteEntity(String url) =>
      http.delete(Uri.parse(url)).then((response) => response.statusCode == 200 ? {} : _handleErrors(response));
