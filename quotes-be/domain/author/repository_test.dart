...
void main() {
  ...
  test("find should return failed Future if db driver throws exception",
      () async {
    when(postgreSQLConnectionMock
            .query(getAuthorStmt, substitutionValues: {"id": authorId}))
        .thenAnswer((_) => Future.error(StateError("exception")));
  ...
  test("update should update and return author entity", () async {
    when(postgreSQLConnectionMock.execute(updateAuthorStmt,
            substitutionValues: updateAuthorParams))
        .thenAnswer((_) => Future.value());

    var result = await authorRepository.update(author);

    verify(postgreSQLConnectionMock.execute(updateAuthorStmt,
        substitutionValues: updateAuthorParams));

    assertAuthor(author, result);
  });

  test("update should return failed Future if db driver throws exception",
      () async {
    when(postgreSQLConnectionMock.execute(updateAuthorStmt,
            substitutionValues: updateAuthorParams))
        .thenAnswer((_) => Future.error(StateError("exception")));
  ...
  test("delete should delete author entity", () async {
    when(postgreSQLConnectionMock
            .execute(deleteAuthorStmt, substitutionValues: {"id": authorId}))
        .thenAnswer((_) => Future.value());
  ...
  test("delete should return failed Future if db driver throws exception",
      () async {
    when(postgreSQLConnectionMock
            .execute(deleteAuthorStmt, substitutionValues: {"id": authorId}))
        .thenAnswer((_) => Future.error(StateError("exception")));
  ...
  test("findAuthors should return authors page", () async {
    when(postgreSQLConnectionMock.query(findAuthorsStmt(searchPhrase),
            substitutionValues: pageParams))
        .thenAnswer((_) => Future.value([authorRow]));

    when(postgreSQLConnectionMock
            .query(findAuthorsCountStmt(searchPhrase), substitutionValues: {}))
        .thenAnswer((_) => Future.value([
              [authorsCount]
            ]));
    ...
    verify(postgreSQLConnectionMock.query(findAuthorsStmt(searchPhrase),
        substitutionValues: pageParams));
  ...
  test("findAuthors should return failed Future if getting authors fails",
      () async {
    when(postgreSQLConnectionMock.query(findAuthorsStmt(searchPhrase),
            substitutionValues: pageParams))
        .thenAnswer((_) => Future.error(StateError("exception")));

    expect(authorRepository.findAuthors(searchPhrase, searchReq),
        throwsStateError);

    verifyNever(postgreSQLConnectionMock.query(findAuthorsStmt(searchPhrase),
        substitutionValues: pageParams));

    verifyNever(postgreSQLConnectionMock
        .query(findAuthorsCountStmt(searchPhrase), substitutionValues: {}));
  });

  test("findAuthors should return failed Future if getting authors count fails",
      () async {
    when(postgreSQLConnectionMock.query(findAuthorsStmt(searchPhrase),
            substitutionValues: pageParams))
        .thenAnswer((_) => Future.value([authorRow]));
