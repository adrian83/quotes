...
const updateQuoteStmt = "UPDATE Quote SET TEXT = @text, " +
...
const listBookQuotesStmt = "SELECT * FROM Quote WHERE BOOK_ID = @bookId " +
...
var findQuotesStmt = (String phrase) =>
    "SELECT * FROM Quote WHERE TEXT " +
    "ILIKE '%$phrase%' ORDER BY CREATED_UTC ASC LIMIT @limit OFFSET @offset";

var findQuotesCountStmt = (String phrase) =>
    "SELECT count(*) FROM Quote WHERE TEXT ILIKE '%$phrase%'";

RowDecoder<Quote> quoteRowDecoder = (List<dynamic> row) => Quote.fromDB(row);

class QuoteRepository extends Repository<Quote> {
  QuoteRepository(PostgreSQLConnection connection)
      : super(connection, quoteRowDecoder);

  Future<Page<Quote>> findBookQuotes(String bookId, PageRequest request) =>
      listAll(listBookQuotesStmt, {
        "limit": request.limit,
        "offset": request.offset,
        "bookId": bookId
      }).then((List<Quote> quotes) =>
          count(bookQuotesCountStmt, {"bookId": bookId})
  ...
  Future<Page<Quote>> findQuotes(String searchPhrase, PageRequest request) {
    ...
    return listAll(findQuotesStmt(params["phrase"]), params).then(
        (List<Quote> quotes) => count(findQuotesCountStmt(params["phrase"]), {})
            .then((total) => PageInfo(request.limit, request.offset, total))
            .then((info) => Page(info, quotes)));
  }
  ...
  Future<Quote> find(String quoteId) =>
      findOneByStatement(getQuoteStmt, {"id": quoteId});

  Future<Quote> update(Quote quote) => updateAtLeastOne(updateQuoteStmt,
      {"id": quote.id, "text": quote.text, "modified": quote.modifiedUtc});

  Future<void> delete(String quoteId) =>
      deleteAtLeastOne(deleteQuoteStmt, {"id": quoteId});
  ...
  Future<void> deleteByBook(String bookId) =>
      deleteAll(deleteBookQuotesStmt, {"bookId": bookId});
}
