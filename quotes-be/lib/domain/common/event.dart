...
abstract class EventRepository<EVENT extends ESDocument, ENTITY extends Entity> {
  ...
  EventRepository(this._store, this._eventDecoder, this._entityDecoder, this._documentCreator, this._documentModifier,
      this._documentDeleter);

  ESStore<EVENT> get store => _store;

  Future<ENTITY> save(ENTITY entity) =>
      Future.value(_documentCreator(entity)).then((event) => store.index(event)).then((_) => entity);

  Future<ENTITY> update(ENTITY entity) =>
      Future.value(_documentModifier(entity)).then((event) => store.index(event)).then((_) => entity);
