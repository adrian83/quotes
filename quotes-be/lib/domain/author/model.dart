...
class Author extends Entity {
  ...
  Author(String id, this._name, this._description, DateTime modifiedUtc, DateTime createdUtc)
      : super(id, modifiedUtc, createdUtc);

  Author.create(this._name, this._description): super(Uuid().v4(), DateTime.now().toUtc(), DateTime.now().toUtc());

  Author.fromJson(Map<String, dynamic> json)
      : this(json[idLabel], json[nameLabel], json[descriptionLabel], DateTime.parse(json[modifiedUtcLabel]),
            DateTime.parse(json[createdUtcLabel]));

  Author.fromDB(List<dynamic> row)
      : this(row[0].toString().trim(), row[1].toString().trim(), row[2].toString().trim(), row[3], row[4]);

  String get name => _name;
  String get description => _description;

  Map toJson() => super.toJson()..addAll({nameLabel: _name, descriptionLabel: _description});
}
...
class AuthorEvent extends ESDocument {
  ...
  AuthorEvent(String eventId, String operation, this._author) : super(eventId, operation);
