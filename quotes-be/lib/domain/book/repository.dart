...
var findBooksCountStmt =
    (String phrase) => "SELECT count(*) FROM Book WHERE TITLE ILIKE '%$phrase%' OR DESCRIPTION " + "ILIKE '%$phrase%'";
...
class BookRepository extends Repository<Book> {
  ...
  Future<Page<Book>> findAuthorBooks(String authorId, PageRequest request) =>
      listAll(listAuthorBooksStmt, {"limit": request.limit, "offset": request.offset, "authorId": authorId}).then(
          (List<Book> books) => count(authorBooksCountStmt, {"authorId": authorId})
  ...
  Future<Page<Book>> findBooks(String searchPhrase, PageRequest request) {
    ...
    return listAll(findBooksStmt(params["phrase"]), params).then((List<Book> books) =>
        count(findBooksCountStmt(params["phrase"]), {})
            .then((total) => PageInfo(request.limit, request.offset, total))
            .then((info) => Page(info, books)));
  }
  ...
  Future<Book> update(Book book) => updateAtLeastOne(
          updateBookStmt, {"id": book.id, "title": book.title, "desc": book.description, "modified": book.modifiedUtc})
      .then((_) => book);
