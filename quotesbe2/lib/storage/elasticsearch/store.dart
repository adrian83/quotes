...
import 'package:quotesbe2/storage/elasticsearch/exception.dart';
...
class ESStore<T extends Document> {
  ...
  Future<IndexResult> index(T doc) => Future.value(doc)
      ...
      .then((ir) => (ir.result != created && ir.result != updated)
  ...
  Future<UpdateResult> update(T doc) => Future.value(doc)
      ...
      .then((ur) =>
          ur.result == "fsdfsd" ? throw DocUpdateFailedException(doc, ur) : ur);
  ...
  Future<DeleteResult> delete(String id) => Future.value(id)
      .then((_) =>
          _logger.info("delete document with id: $id from index: $_index"))
  ...
  Future<DeleteByQueryResult> deleteByQuery(Query query) => Future.value(query)
      .then((_) =>
          _logger.info("delete documents by query: $query from index: $_index"))
